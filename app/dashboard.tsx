...
type LocalCode = "RASTER" | "DRYFT" | "MR_TASTY";
type Theme = { code: LocalCode; name: string; logo: string; subtitle: string; background: string; panel: string; border: string; primary: string; secondary: string; accent: string; text: string };

const themes: Record<LocalCode, Theme> = {
  RASTER: { code: "RASTER", name: "RASTER", logo: "/raster-logo.png", subtitle: "BAR & CLUB // CONTROL CENTER", background: "#080b10", panel: "#0e131a", border: "#193641", primary: "#16e0ff", secondary: "#ec35e9", accent: "#f8e515", text: "#e7fbff" },
  DRYFT: { code: "DRYFT", name: "DRYFT", logo: "/dryft-logo.png", subtitle: "GESTIÓN OPERATIVA", background: "#1b101a", panel: "#281526", border: "#63304e", primary: "#f3b9da", secondary: "#d56caa", accent: "#f5c85b", text: "#fff5fb" },
  MR_TASTY: { code: "MR_TASTY", name: "MR TASTY", logo: "/mr-tasty-logo.jpeg", subtitle: "FOOD & CONTROL", background: "#252525", panel: "#302f2d", border: "#554a3e", primary: "#ffad18", secondary: "#f27616", accent: "#ffe0a1", text: "#fff8e9" },
};
...
export default function Dashboard({ user, signOutPath }: Props) {
  ...
  const [activeLocal, setActiveLocal] = useState<LocalCode>("RASTER");
  const [open, setOpen] = useState(false);
  const current = items.find((item) => item.id === active) ?? items[0];
  const theme = themes[activeLocal];
  ...
  return (
    <main className="min-h-screen text-[#e7fbff] selection:bg-[#ec35e9] selection:text-white" data-site-theme={activeLocal} style={{ backgroundColor: theme.background, color: theme.text }}>
      <div className="pointer-events-none fixed inset-0 opacity-[.08] scanlines" />
      <div className="flex min-h-screen">
        <aside className={`fixed inset-y-0 left-0 z-30 flex w-[280px] flex-col border-r-2 transition-transform lg:static lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`} style={{ backgroundColor: theme.panel, borderColor: theme.border }}>
          <div className="border-b-2 border-[#193641] px-5 pb-4 pt-3">
            <div className="relative mx-auto h-[130px] w-[150px] overflow-hidden border-2 border-[#263841] bg-[#0b0e13] shadow-[4px_4px_0_#06080c]">
              <img src={theme.logo} alt={`${theme.name} logo`} className="h-full w-full object-cover" />
            </div>
            <div className="mt-3 flex items-center justify-between"><div><p className="font-mono text-[15px] font-black uppercase tracking-[.08em]" style={{ color: theme.primary }}>{theme.name}</p><p className="font-mono text-[10px] font-bold uppercase tracking-[.14em]" style={{ color: theme.accent }}>{theme.subtitle}</p></div><button className="lg:hidden" onClick={() => setOpen(false)} aria-label="Cerrar menú"><X size={18} /></button></div>
          </div>
        ...
        <section className="min-w-0 flex-1">
          <header className="flex h-[82px] items-center justify-between border-b-2 px-5 sm:px-8" style={{ backgroundColor: theme.panel, borderColor: theme.border }}><div className="flex items-center gap-3"><button className="border-2 border-[#314c56] p-2 lg:hidden" style={{ color: theme.primary }} onClick={() => setOpen(true)} aria-label="Abrir menú"><Menu size={18} /></button><div><p className="font-mono text-[9px] font-black uppercase tracking-[.18em]" style={{ color: theme.secondary }}>SECTOR // {current.group}</p><h1 className="font-mono text-lg font-black uppercase tracking-wide" style={{ color: theme.accent }}>{current.label}</h1></div></div><div className="flex items-center gap-3"><label className="hidden items-center gap-2 border-2 px-3 py-2 sm:flex" style={{ borderColor: theme.border, backgroundColor: theme.background }}><span className="font-mono text-[9px] font-bold uppercase" style={{ color: theme.primary }}>Local activo</span><select value={activeLocal} onChange={(event) => setActiveLocal(event.target.value as LocalCode)} className="bg-transparent font-mono text-[10px] font-black uppercase outline-none" style={{ color: theme.accent }}><option value="RASTER">RASTER</option><option value="DRYFT">DRYFT</option><option value="MR_TASTY">MR TASTY</option></select></label><div className="hidden items-center gap-2 border-2 px-3 py-2 sm:flex" style={{ borderColor: theme.border, backgroundColor: theme.background }}><span className="h-2 w-2 animate-pulse bg-[#62ef87]" /><span className="font-mono text-[9px] font-bold uppercase text-[#83aab4]">Sistema online</span></div><button className="relative border-2 p-2" style={{ borderColor: theme.border, color: theme.primary }} aria-label="Notificaciones"><Bell size={17} /><span className="absolute -right-1 -top-1 h-2 w-2" style={{ backgroundColor: theme.secondary }} /></button></div></header>
          <div className="mx-auto max-w-[1440px] p-5 sm:p-8">{active === "home" ? <HomeView navigate={navigate} theme={theme} /> : active === "stock" ? <StockView theme={theme} /> : <ModuleView item={current} />}</div>
        </section>
      </div>
    </main>
  );
...
function HomeView({ navigate, theme }: { navigate: (id: string) => void; theme: Theme }) {
  return <div className="space-y-7"><div className="flex flex-col justify-between gap-5 border-b-2 border-dashed border-[#203a44] pb-6 md:flex-row md:items-end"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.16em] text-[#62828c]">LOCAL ACTIVO // {theme.name}</p><h2 className="mt-3 font-mono text-3xl font-black uppercase tracking-[-.05em] sm:text-[40px]" style={{ color: theme.primary }}>Base principal</h2><p className="mt-2 max-w-2xl font-mono text-xs leading-6 text-[#8aabb3]">Centro de control para {theme.name}, stock, compras, proveedores, producción y administración.</p></div><button onClick={() => navigate("purchases")} className="w-fit border-2 px-4 py-3 font-mono text-xs font-black uppercase shadow-[4px_4px_0_#ec35e9] transition" style={{ borderColor: theme.accent, backgroundColor: theme.accent, color: theme.background }}>+ Nueva compra</button></div>
...
function StockView({ theme }: { theme: Theme }) {
  ...
  return <div className="space-y-6"><div className="flex flex-col justify-between gap-4 border-b-2 border-dashed border-[#203a44] pb-6 md:flex-row md:items-end"><div><p className="pixel-label">{theme.name} // OPERACIONES // STOCK</p><h2 className="mt-2 font-mono text-3xl font-black uppercase" style={{ color: theme.primary }}>Movimientos de stock</h2><p className="mt-2 font-mono text-xs text-[#8aabb3]">Carga de ingresos, producción, decomisos, ventas y comida personal.</p></div><div className="border-2 border-[#62ef87] bg-[#10291d] px-4 py-2 font-mono text-[10px] font-bold uppercase text-[#62ef87]">Base conectada</div></div>
